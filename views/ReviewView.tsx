...
const COLORS = ['#1e293b', '#90C030', '#3b82f6'];

export const ReviewView: React.FC<{ onBack: () => void }> = ({ onBack }) => {
    ...
    <div className="max-w-6xl mx-auto px-4 py-8">
      ...
      <header className="bg-white p-8 rounded-2xl shadow-sm border-t-4 border-ariel-green mb-8 border border-slate-100 relative">
        <div className="flex flex-col md:flex-row justify-between items-start gap-6">
          <div className="flex-grow">
            <h1 className="text-3xl font-black text-gray-800 mb-2">ניהול תורים במיון: סקירה שיטתית</h1>
          ...
          <div className="grid grid-cols-2 gap-4 w-full md:w-auto">
            <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 text-center min-w-[120px]">
              ...
              <div className="text-2xl font-black text-slate-800">229</div>
            </div>
            <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 text-center min-w-[120px]">
      ...
      <div className="bg-white rounded-3xl p-8 shadow-xl mb-12 border border-slate-100 overflow-hidden relative">
        <div className="flex flex-col md:flex-row justify-between items-center mb-10 gap-4">
          <div>
            <h3 className="text-2xl font-black text-slate-800">דשבורד התפתחות מתודולוגית</h3>
          ...
          <div className="flex bg-slate-100 p-1 rounded-xl">
            <button 
              ...
              className={`px-6 py-2 rounded-lg text-sm font-bold transition-all ${viewMode === 'grid' ? 'bg-white text-ariel-green shadow-sm' : 'text-slate-500'}`}
            >
              <i className="fas fa-th-large ml-2"></i> השוואת עשורים
            </button>
            <button 
              onClick={() => setViewMode('trend')}
              className={`px-6 py-2 rounded-lg text-sm font-bold transition-all ${viewMode === 'trend' ? 'bg-white text-ariel-green shadow-sm' : 'text-slate-500'}`}
            >
        ...
        {viewMode === 'grid' ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6">
            {REVIEW_TIMELINE.map((item) => (
              <div key={item.year} className="bg-slate-50 rounded-2xl p-6 border border-slate-200 flex flex-col items-center group hover:border-ariel-green transition-all hover:shadow-md min-h-[440px]">
                <div className="text-lg font-black text-slate-400 group-hover:text-ariel-green mb-4">{item.year}s</div>
                ...
                <div className="text-center flex-grow flex flex-col justify-start w-full">
                  <div className="text-sm font-bold text-slate-800 mb-3 leading-tight border-b border-slate-200 pb-2">
          ...
          <div className="h-[400px] w-full pt-4">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={trendData}>
                ...
                />
                <Area type="monotone" dataKey="Analytical" name="אנליטי" stackId="1" stroke="#1e293b" fill="#1e293b" fillOpacity={0.2} />
                <Area type="monotone" dataKey="Simulation" name="סימולציה" stackId="1" stroke="#90C030" fill="#90C030" fillOpacity={0.2} />
                <Area type="monotone" dataKey="AI_ML" name="AI/ML" stackId="1" stroke="#3b82f6" fill="url(#colorAI)" fillOpacity={1} />
              </AreaChart>
            </ResponsiveContainer>
            <div className="flex justify-center gap-6 mt-6">
              <div className="flex items-center gap-2 text-xs font-bold text-slate-600"><div className="w-3 h-3 bg-[#1e293b] rounded-sm"></div> מודלים אנליטיים</div>
              <div className="flex items-center gap-2 text-xs font-bold text-slate-600"><div className="w-3 h-3 bg-[#90C030] rounded-sm"></div> סימולציה</div>
