...
export const FloatingView: React.FC<{ onBack: () => void }> = ({ onBack }) => {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8 font-sans text-right" dir="rtl">
      ...
      <header className="bg-white p-8 rounded-2xl shadow-sm border-t-4 border-ariel-green mb-8 border border-slate-100 relative">
        <div className="flex flex-col md:flex-row justify-between items-start gap-6">
          <div className="flex-grow">
            <h1 className="text-3xl font-black text-gray-800 mb-2 font-display">שיטת "המטופל הצף" (Floating Patients)</h1>
          ...
          <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 text-center min-w-[140px]">
            ...
            <div className="text-2xl font-black text-slate-800 tracking-tight">2015-2017</div>
      ...
      <div className="bg-white p-6 rounded-[2.5rem] shadow-sm border border-slate-100 mb-12">
        ...
        <div className="grid lg:grid-cols-4 gap-4 mb-4">
          <div className="bg-white p-4 rounded-2xl shadow-sm border border-slate-100 flex flex-col justify-between">
            <div>
              <h3 className="text-sm font-black text-slate-800 mb-3 tracking-tight">הגדרות סימולציה</h3>
              <div className="space-y-4">
                <div>
                  <div className="flex justify-between text-[10px] font-bold mb-1">
                    <span>קצב הגעה</span>
                    <span className="text-ariel-green">{(arrivalRate/1000).toFixed(1)} ש'</span>
                  </div>
                  <input type="range" min="1000" max="8000" step="500" value={arrivalRate} onChange={(e) => setArrivalRate(Number(e.target.value))} className="w-full accent-ariel-green h-1 bg-slate-100 rounded-full" />
                </div>
                <div>
                  <div className="flex justify-between text-[10px] font-bold mb-1">
            ...
            <button 
              ...
              className={`mt-4 w-full py-2 rounded-xl text-xs font-bold transition-all ${isAutoRunning ? 'bg-red-50 text-red-500 border border-red-100' : 'bg-ariel-green text-white shadow-md'}`}
            >
              {isAutoRunning ? 'עצור הזרמה' : 'הפעל הזרמת חולים'}
            </button>
          </div>

          <div className="bg-slate-900 p-4 rounded-2xl shadow-xl text-white lg:col-span-2">
            <div className="flex justify-between items-center mb-4">
               <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest">מדדי עומס מערכתיים</h3>
               <div className="text-[10px] text-ariel-green font-mono">ΔC: {Math.abs((edPatients.length/MAX_ED_BEDS) - (wardOccupancy/100)).toFixed(2)}</div>
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-slate-800 p-3 rounded-xl border border-slate-700">
                 <div className="text-[10px] text-slate-500 font-bold mb-1">עומס מחלקות רקע</div>
                 <input type="range" value={wardOccupancy} onChange={(e) => setWardOccupancy(Number(e.target.value))} className="w-full accent-ariel-green h-1 bg-slate-900 mb-2" />
                 <div className="text-xl font-black">{wardOccupancy}%</div>
              </div>
              <div className="bg-slate-800 p-3 rounded-xl border border-slate-700">
                 <div className="text-[10px] text-slate-500 font-bold mb-1">יומן אירועים</div>
                 <div className="space-y-1 mt-1">
                   {logs.map((log, i) => (
                     <div key={i} className={`text-[9px] truncate ${i===0 ? 'text-white font-bold' : 'text-slate-500'}`}>{log}</div>
                   ))}
                 </div>
              </div>
            </div>
      ...
      <div className="bg-white p-10 rounded-3xl shadow-sm border border-slate-200">
        <h3 className="text-2xl font-black text-ariel-green mb-8 flex items-center font-display">
          <i className="fas fa-file-medical ml-3 text-ariel-green"></i> גוף המחקר וניתוח אלגוריתמי
        </h3>
        ...
        <div className="space-y-8 text-lg text-slate-700 leading-relaxed text-justify">
          ...
          <p>
            לסיכום, המחקר מוכיח כי ניהול עומסים אינו בעיה "מקומית" של חדר המיון אלא אתגר רשתי הדורש סנכרון אלגוריתמי בין כלל מחלקות בית החולים. יישום של מערכות תומכות החלטה מבוססות חקר ביצועים, כפי שמוצג בפורטל זה, מהווה תשתית הכרחית לבניית בתי חולים חכמים ויעילים יותר בעתיד.
          </p>
        </div>
      </div>
    </div>
