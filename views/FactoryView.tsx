...
export const FactoryView: React.FC<{ onBack: () => void }> = ({ onBack }) => {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8">
      ...
      <header className="bg-white p-8 rounded-2xl shadow-sm border-t-4 border-ariel-green mb-8 border border-slate-100 relative text-right">
        <div className="flex flex-col md:flex-row justify-between items-start gap-6">
          <div className="flex-grow">
            <h1 className="text-3xl font-black text-gray-800 mb-2 font-display">אופטימיזציית הקצאה רב-מפעלית (2020)</h1>
            <p className="text-slate-500 font-medium mb-4 italic">Journal of Industrial and Production Engineering</p>
            <div className="flex flex-wrap gap-3">
              <a href="https://doi.org/10.1080/21681015.2020.1801867" target="_blank" rel="noreferrer" className="bg-ariel-green text-white px-4 py-2 rounded-lg text-sm font-bold hover:bg-green-600 transition-colors flex items-center gap-2 shadow-md">
          ...
          <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 text-center min-w-[160px]">
            ...
            <div className="text-2xl font-black text-ariel-green tracking-tight">O(n³/ε)</div>
      ...
      <div className="bg-slate-900 text-white rounded-[2.5rem] p-8 mb-12 shadow-2xl border border-slate-800 relative overflow-hidden">
        <div className="absolute top-0 right-0 w-64 h-64 bg-ariel-green/5 rounded-full blur-3xl -mr-32 -mt-32"></div>
        
        <div className="flex flex-col xl:flex-row justify-between items-center mb-10 gap-8 relative z-10">
          <div className="flex items-center gap-4 text-right">
             <div className="w-14 h-14 bg-ariel-green/20 rounded-2xl flex items-center justify-center text-ariel-green text-3xl shadow-inner">
          ...
          <div className="flex flex-wrap justify-center items-center gap-6 bg-slate-800/40 backdrop-blur-md p-5 rounded-[2rem] border border-slate-700/50">
             <div className="text-center px-4">
               <div className="text-[10px] text-slate-500 uppercase font-black mb-2">דיוק (ε) - מרחק מאופטימום</div>
               <div className="flex items-center gap-4">
                 <input 
                    type="range" min="0.01" max="0.2" step="0.01" value={epsilon} 
                    onChange={(e) => setEpsilon(Number(e.target.value))}
                    className="w-32 accent-ariel-green h-1.5 bg-slate-700 rounded-full" 
                  />
                 <span className="text-sm font-mono font-bold text-ariel-green bg-slate-900 px-2 py-1 rounded">{(epsilon * 100).toFixed(0)}%</span>
               </div>
             </div>
             <div className="hidden md:block w-px h-10 bg-slate-700"></div>
             <div className="text-center px-4">
               <div className="text-[10px] text-slate-500 uppercase font-black mb-1">רווח שנצבר (S)</div>
               <div className="text-3xl font-black text-white tracking-tighter">
                  <span className="text-ariel-green">$</span>{totalProfit.toFixed(1)}<span className="text-lg ml-1">K</span>
               </div>
             </div>
             <button 
               onClick={runScheduling} disabled={isSimulating}
               className="bg-ariel-green hover:bg-green-600 disabled:opacity-50 text-white font-black py-4 px-10 rounded-2xl transition-all shadow-[0_0_20px_rgba(144,192,48,0.3)] flex items-center gap-3 text-sm uppercase tracking-widest"
             >
        ...
        <div className="grid lg:grid-cols-12 gap-8 items-stretch">
          ...
          <div className="lg:col-span-4 flex flex-col h-full overflow-hidden">
             <div className="flex justify-between items-center px-2 mb-4">
                <h4 className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] text-right">משימות ממתינות (n=10)</h4>
                <button 
                  onClick={randomizeData} 
                  disabled={isSimulating}
                  className="text-[9px] font-bold text-ariel-green hover:text-white transition-colors flex items-center gap-1"
                >
             ...
             <div className="space-y-1.5 custom-scrollbar overflow-y-auto pr-2 flex-grow max-h-[580px]">
                {jobs.map(job => (
                  <div key={job.id} className={`py-1.5 px-3 rounded-xl border transition-all duration-500 flex flex-col justify-center ${job.status === 'main' ? 'bg-ariel-green/10 border-ariel-green/40' : (job.status === 'sub' ? 'bg-blue-500/10 border-blue-500/40' : (job.status === 'late' ? 'bg-red-500/10 border-red-500/40 opacity-50' : 'bg-slate-800 border-slate-700'))}`}>
                    <div className="flex justify-between items-center mb-0.5">
                      <span className="text-[9px] font-black text-slate-100 uppercase">פרויקט #{job.id}</span>
                      <span className={`text-[7px] font-black px-1.5 py-0.5 rounded uppercase tracking-tighter ${job.status === 'main' ? 'bg-ariel-green text-slate-900' : (job.status === 'sub' ? 'bg-blue-500 text-white' : (job.status === 'late' ? 'bg-red-500 text-white' : 'bg-slate-700 text-slate-400'))}`}>
          ...
          <div className="lg:col-span-8 flex flex-col h-full">
             <h4 className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] mb-6 text-right">מפת קיבולת רשת (Multi-Factory Map)</h4>
             <div className="grid grid-cols-1 md:grid-cols-2 gap-6 flex-grow">
                {factories.map(f => (
                  <div key={f.id} className="bg-slate-800/40 p-6 rounded-[2rem] border border-slate-700/50 flex flex-col group hover:border-ariel-green/30 transition-all text-right">
                    <div className="flex justify-between items-start mb-4">
                       <div className="flex items-center gap-3">
                          <div className={`w-10 h-10 rounded-xl flex items-center justify-center text-xl ${f.type === 'high' ? 'bg-ariel-green/20 text-ariel-green' : 'bg-slate-700 text-slate-400'}`}>
                             <i className={`fas ${f.icon}`}></i>
                          </div>
                          <div className="text-right">
                             <div className="text-sm font-black text-white">{f.name}</div>
                             <div className="text-[9px] text-slate-500 uppercase font-bold tracking-widest">{f.type === 'high' ? 'High Efficiency' : 'Sub-Contractor'}</div>
                          </div>
                       </div>
                       <div className="text-right">
                          <div className="text-[14px] font-mono font-bold text-slate-300">
                             {f.currentLoad.toFixed(1)}<span className="text-[10px] text-slate-500 ml-1">/ {f.capacity}h</span>
                          </div>
                       </div>
                    </div>
                    ...
                    <div className="mt-auto">
                       <div className="h-3 w-full bg-slate-950 rounded-full overflow-hidden p-[2px]">
                          <div 
                            className={`h-full rounded-full transition-all duration-1000 ease-out ${f.currentLoad > f.capacity ? 'bg-red-500' : (f.currentLoad > f.capacity * 0.8 ? 'bg-orange-500' : 'bg-ariel-green')}`}
                            style={{ width: `${Math.min(100, (f.currentLoad / f.capacity) * 100)}%` }}
                          />
                       </div>
                       <div className="flex justify-between mt-2 px-1">
                          <span className="text-[9px] text-slate-500 font-bold">עומס: {Math.floor((f.currentLoad / f.capacity) * 100)}%</span>
             ...
             <div className="mt-8 grid md:grid-cols-3 gap-6">
                <div className="bg-slate-800/20 p-4 rounded-2xl border border-slate-800 text-right">
                   <div className="text-[10px] text-ariel-green uppercase font-black mb-2 flex items-center gap-2 justify-end">
      ...
      <div className="bg-white p-10 rounded-3xl shadow-sm border border-slate-200 text-right">
        <h3 className="text-2xl font-black text-ariel-green mb-8 flex items-center justify-end font-display">
          תקציר המאמר וניתוח ממצאים <i className="fas fa-file-alt mr-3 text-ariel-green"></i>
        </h3>
